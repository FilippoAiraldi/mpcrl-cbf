...
from mpcrl.util.control import lqr
...
def lie_derivative(
    ex: cs.SX | cs.MX, arg: cs.SX | cs.MX, field: cs.SX | cs.MX
) -> cs.SX | cs.MX:
    return cs.dot(cs.gradient(ex, arg), field)
...
dynamics = env.A @ x + env.B @ u

center1 = (0.05, 0.3)
radii1 = (0.05, 0.1)
h1 = (
    (x[0] - center1[0]) ** 2 / radii1[0] ** 2
    + (x[1] - center1[1]) ** 2 / radii1[1] ** 2
    - 1
)
h1_dot = lie_derivative(h1, x, dynamics)
cbf_constraint1 = h1_dot + alpha(h1)

center2 = (0, 0.4)
radii2 = (0.15, 0.5)
h2 = (
    1
    - (x[0] - center2[0]) ** 2 / radii2[0] ** 2
    - (x[1] - center2[1]) ** 2 / radii2[1] ** 2
)
h2_dot = lie_derivative(h2, x, dynamics)
cbf_constraint2 = h2_dot + alpha(h2)
